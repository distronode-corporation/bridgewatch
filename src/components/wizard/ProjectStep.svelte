...
<div class="flex flex-col gap-4">
  {#if loading}
    <p class="text-muted-foreground text-sm" aria-live="polite">Loading projects…</p>
  {:else if loadError}
    <Alert.Root variant="destructive">
      <Alert.Title>Could not list projects</Alert.Title>
      <Alert.Description>{loadError} Type the project below instead.</Alert.Description>
    </Alert.Root>
  ...
  {:else if listing?.mode === "projects"}
    <div class="flex flex-col gap-2">
      <div class="flex gap-2">
        <Label for="wizard-project-search" class="sr-only">Search projects</Label>
        <Input
      ...
      {#if projects.length === 0}
        <p class="text-muted-foreground text-sm">No projects match.</p>
      {:else}
          ...
          {#each projects as project (project.id)}
            <label class="hover:bg-muted flex items-center gap-2 rounded-md px-2 py-1 text-sm">
              <input
                ...
                checked={draft.project?.ref === project.id}
        ...
        {#if listing.truncated}
          <p class="text-muted-foreground text-xs">More projects than shown; search to narrow the list.</p>
        {/if}
      {/if}
    </div>
  {/if}
  ...
  <div class="flex flex-col gap-1">
    <Label for="wizard-project-input">Project id, path or URL</Label>
    <div class="flex gap-2">
      <Input
