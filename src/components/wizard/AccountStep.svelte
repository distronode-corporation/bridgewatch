<script lang="ts">
  /** Step 1: instance, token source, "Test connection". */
  ...
  import type { GlabDetection, Identity } from "./api";
  import { type Draft, type StepErrors, type TokenMode } from "./model";

  interface Props {
    draft: Draft;
    errors: StepErrors;
    glab: GlabDetection | null;
    identity: Identity | null;
    testing: boolean;
    testError: string | null;
    onTest: () => void;
    /** The instance address settled (radio change or URL blur): re-detect glab. */
    onInstanceChange: () => void;
  }

  let { draft = $bindable(), errors, glab, identity, testing, testError, onTest, onInstanceChange }: Props = $props();

  /** Offer the keyring option when detection found glab's item, or when re-running on a config that already uses one. */
  const glabFound = $derived(glab?.status === "found" || draft.glabSource !== null);

  const MODES: { value: TokenMode; label: string; hint: string }[] = [
    { value: "glab", label: "Use glab's token", hint: "Found in the system keychain; bridgewatch reads it where glab keeps it." },
    { value: "paste", label: "Paste a token", hint: "Stored in bridgewatch's own keychain entry, never in config.toml." },
    { value: "env", label: "Environment variable", hint: "Only the variable's NAME goes in the file." },
    { value: "command", label: "Run a command", hint: "Its output is the token, e.g. pass gitlab/pat. You will be asked before it runs." },
  ];
  ...
  function selectMode(mode: TokenMode) {
    ...
    if (mode === "glab" && glab?.status === "found") draft.glabSource = glab.source;
  }
</script>

<div class="flex flex-col gap-4">
  <fieldset class="flex flex-col gap-2">
    <legend class="mb-1 text-sm font-medium">GitLab instance</legend>
    <label class="flex items-center gap-2 text-sm">
      <input
        ...
        checked={draft.instance === "gitlab.com"}
        onchange={() => {
          draft.instance = "gitlab.com";
          onInstanceChange();
        }}
      />
      gitlab.com
    ...
    {#if draft.instance === "self-managed"}
      <div class="flex flex-col gap-1 pl-6">
        ...
        />
        <FieldMessage id="wizard-base-url-msg" message={errors.base_url} />
      </div>
    {/if}
  </fieldset>

  <fieldset class="flex flex-col gap-2" aria-describedby="wizard-token-msg">
    <legend class="mb-1 text-sm font-medium">Token</legend>
    {#each MODES as mode (mode.value)}
      {#if mode.value !== "glab" || glabFound}
    ...
    <div class="pl-6">
      {#if draft.tokenMode === "paste"}
        ...
        <Input
          ...
          autocomplete="off"
          spellcheck={false}
          placeholder={draft.ownStored ? "Leave empty to keep the stored token" : "glpat-…"}
